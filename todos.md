here are all the todos i can thing about so far:


todo:
- set custom colour button in token type to first instead of last, so once theres al ot of colour u dont have to seek it
- find a way to be able to add/remove lexer after compile time, not onlu during
- if a lexer is changed/deleted its settings should be removed to, i.e if not in lexers at loaddata
- when a new colour is added to the pallete, it should imdiatly appear for the lexers too (i.e syntc lexers and pallete somehow)
- forbid deleteing colours that are in use
- also might wanna add a confirmation menu to deleting a colour